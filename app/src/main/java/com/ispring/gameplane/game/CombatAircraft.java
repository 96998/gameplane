...
public class CombatAircraft extends Sprite {
    ...
    public void fight(GameView gameView){
        ...
        if(single){
            ...
            //捡拾到双发奖励后,双发模式下发射两发蓝色子弹
            float offset = getWidth() / 4;
            float leftX = x - offset;    //左边子弹的发射位置
            float rightX = x + offset;   //右边子弹的发射位置
    ...
    protected void afterDraw(Canvas canvas, Paint paint, GameView gameView){
        ...
        if(!collide){
            ...
            for(EnemyPlane enemyPlane : enemies){
                ...
                if(lives==0){
                    //p为战斗机与敌机的碰撞点，如果p不为null，则表明战斗机被敌机击中,碰撞后敌我飞机都会炸裂
        ...
        if(beginFlushFrame > 0){
            ...
            if(frame >= beginFlushFrame){
                if((frame - beginFlushFrame) % flushFrequency == 0){
                    ...
                    flushTime++;
                    if(flushTime >= maxFlushTime&&lives==0){
                        //如果战斗机闪烁的次数超过了最大的闪烁次数且飞机没额外生命，那么销毁战斗机
                        destroy();
                        //Game.gameOver();
                    }
                    else if(flushTime >= maxFlushTime&&lives!=0){
                        draw(canvas, paint, gameView);
                    }
                }
            }
        }
